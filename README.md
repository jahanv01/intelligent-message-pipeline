...
# Export small, safe, mandatory-ID-scoped copies of priority/group/routing
# output to sample_outputs/ (committed) -- see "Why sample_outputs/ exists" below
python3 submission_tests/export_sample_outputs.py
```
...
### Why `sample_outputs/` exists

`results/` — the full run over the real 1104-message dataset — is git-ignored on purpose: publishing it in full would mean publishing near-verbatim excerpts of most of the supplied dataset (group titles and task descriptions are lightly-cleaned copies of the original message text), which the assignment explicitly prohibits. But the assignment *also* requires submitting a priority output file, a related-message group output file, and a privacy-routing output file as deliverables. `sample_outputs/` (committed, NOT git-ignored) resolves that: it holds small copies scoped only to the 15 mandatory L1 message IDs and/or the 24-message L2 demo batch — the exact messages the assignment says must appear in the video anyway — generated by `submission_tests/export_sample_outputs.py` and the other `submission_tests/*.py` scripts. Every file in it was checked for leaked sensitive values before being committed (`masked_text` only, never a raw value — see `sensitive.py`).
...
92 tests cover classification, extraction, sensitive detection, pipeline integration, timestamp parsing, message linking, priority scoring, related-message grouping, privacy routing, and the semantic assistant. All tests use fabricated example messages — the real dataset is never used in tests.
...
├── results/                 ← git-ignored — all output JSON files written here (full dataset)
├── sample_outputs/           ← COMMITTED — small, mandatory-ID-scoped output samples (see below)
